 ...
 * @param args
 */
export function numberSum(...args) {
 ...
 * @returns
 */
export function fixedNumberAdd(...args) {
 ...
 * @param end 结束加密索引
 */
export function numberEncrypt(num, start = 3, end = -4) {
    if (!Number(num))
        return;
    let password = '';
 ...
 * @returns
 */
export function toFixed2AndBankCount(num = '', fixed = 2) {
 ...
 * @returns
 */
export function numberPercentage(num, digit = 0) {
    if (!Number(num))
        return;
